...
class DashboardView(LoginRequiredMixin, AdminTeacherRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['pending_lessons'] = Lesson.objects.filter(is_approved=False).count()
        context['pending_tests'] = Test.objects.filter(is_approved=False).count()
        context['pending_resources'] = Resource.objects.filter(is_approved=False).count()
        context['total_pending_content'] = context['pending_lessons'] + context['pending_tests'] + context['pending_resources']

        # Recent Activity
        context['recent_users'] = User.objects.order_by('-date_joined')[:5]
        context['recent_lessons'] = Lesson.objects.order_by('-created_at')[:5]
...
class ContentManagementView(LoginRequiredMixin, AdminTeacherRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if content_type == 'test':
            ...
            queryset = Lesson.objects.all().select_related('author')
