...
def student_dashboard(request):
    ...
    pending_lessons = Lesson.objects.filter(author=request.user, is_approved=False).order_by('-created_at')
    pending_tests = Test.objects.filter(author=request.user, is_approved=False).order_by('-created_at')
    pending_resources = Resource.objects.filter(author=request.user, is_approved=False).order_by('-created_at')
...
def chat_room(request, year, stream):
    ...
    chat_messages = ChatMessage.objects.filter(year=year, stream=stream)
...
def delete_chat_message(request, pk):
    if request.method == 'POST':
        message = get_object_or_404(ChatMessage, pk=pk)
        # Author can delete, or staff/teachers
...
class LessonDeleteView(LoginRequiredMixin, UserPassesTestMixin, DeleteView):
    model = Lesson
    success_url = reverse_lazy('lesson_list')
    template_name = 'content/lesson_running_delete.html' # Doesn't actually use a template if we handle post correctly or get, but standard DeleteView expects one or confirm
    
    # We'll use a custom test func
    def test_func(self):
        lesson = self.get_object()
        return self.request.user.is_staff or self.request.user == lesson.author

    # Bypass confirmation template for now to match behaviour (GET request deletes? Not safe but matching existing)
    # Actually existing uses a GET request to delete with a JS confirm.
    # Standard DeleteView requires POST.
    # To keep compatibility with existing "GET" link, we can override get to post.
